  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    async execute(args: { story: string; search_topics?: string; segments?: number }) {
      ...
      return (
        ...
        `⑫ 时长与语速：每段时长是否在 8–13 秒区间（不低于 8、不超过 13）；对白是否按 3–4 字/秒 自然语速规划；有无为凑固定时长而加速台词或拖慢动作；对白多时是否给了足额时长或合理拆分。\n` +
        `⑬ 人物资产详细度：公共人物绑定区的每个 <Subject N> 是否达到"角色资产圣经"级别（身份/年龄/体型/面部/皮肤/发型/手部职责表/服装/气质/目标十项齐全，非一句话压缩）；有无把人物写成一两句简略概述。\n` +
        `发现任何冲突（如时长低于8秒或超过13秒、Picture 遗漏、字段名松动、动作倒带、位置瞬移、镜头跳切、物理不合理、恢复默认状态、对白缺失<d>标签、同人物分身/双像/瞬移、原对白遗漏或改写、未出场人物误入画面、对白语速过快），必须先自行重新设计对应 Segment，再输出。\n` +
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    async execute(args: { novel_chapter: string; preserve_dialog?: boolean; target_seconds?: number }) {
      ...
      return (
        ...
        `- 版本裁决：本任务包末尾《影视分镜  4.0》中与上述"8-14秒/镜头数不定/新Segment格式"冲突的旧表述，一律以规范内【版本裁决（新规则优先）】与本节为准；其他规则（忠于原著/原对白/场景Beat/轴线/完整性检查）继续有效。\n\n` +
        `# 二、小说章节原文（用户提供）\n\n${chapter}\n\n` +
