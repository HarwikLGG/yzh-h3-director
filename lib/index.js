	...
	try {
		...
		*/
		...
		ctx.on("tools/post-execute", async (exec, result, next) => {
			...
			const transformed = await transformContent(ctx, credentialRefFn, launchEnvOfFn, resolved, content, exec.signal, { count: 0 });
			if (transformed === null) return decision;
			ctx.logger.info("vision-bridge: 工具 %s 的结果含图片，已替换为视觉模型（%s）的文本描述", exec.name, resolved.model);
			return {
				kind: "accept",
				content: transformed,
