...
* 块会直接以 `UNSUPPORTED_CONTENT` 拒绝。本插件在 `agent/pre-step` 瀑布里
* 拦截即将进入请求的用户消息：把其中的 `image` 内容块交给用户指定的
* OpenAI 兼容视觉 API（chat/completions + image_url data URL）识别，得到
* 文本描述后，用描述文本块原位替换图片块，再交给主模型（DeepSeek）。
* 主模型因此"看得到"图片内容，而适配器只收到文本。
*
...
async function callVision(ctx, credentialRefFn, launchEnvOfFn, options, attachment, data, signal, index, total) {
	...
	const effectiveSignal = AbortSignal.any([signal, AbortSignal.timeout(options.timeoutMs)]);
	let response;
...
* 输出 JPEG。返回 { data, mediaType }；sips 不可用/失败时返回 null（调用方
* 决定降级策略）。GIF 取第一帧；PNG/WebP/JPEG 均支持输入。
*/
function downscaleImage(data, mediaType, maxDimension) {
	let dir = null;
	try {
		...
		// sips 不存在或处理失败 → 返回 null，调用方按原图发送并记日志
		return null;
	} finally {
		if (dir !== null) {
			try {
...
/** 单张图片的描述文本（含失败占位逻辑）。 */
async function describeOne(ctx, credentialRefFn, launchEnvOfFn, options, attachment, signal, index, total) {
	...
	if (pixels > options.maxVisionPixels) {
		const scaled = downscaleImage(sendData, attachment.mediaType, options.visionMaxDimension);
		if (scaled !== null) {
			...
			ctx.logger.warn(
				"vision-bridge: 图片 %s 像素 %d 超过上限且缩小失败（sips 不可用），按原图发送，视觉服务端可能拒绝",
				label,
				pixels
			);
		}
...
function apply(ctx, config) {
	try {
		...
		// inputModalities 让图片进入消息管线；随后 pre-step 转换保证真正发给
		...
		ctx.on(
			...
			async ({ signal }, next) => {
				...
				ctx.logger.info("vision-bridge: 已将消息中的图片替换为视觉模型（%s）的文本描述", resolved.model);
