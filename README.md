...
- 六字段提示词：第 17 技能「连续剧情导演版」（官方 Context-IR 六字段修正版），`src/prompt.ts` 由 `连续剧情导演版_最终版.txt` 原封不动生成（MD5 校验一致）；**v0.0.2 实战修正**：追加第六十一节《稳定复现硬规则》，并修订生成要求/审核轮
...
v0.0.2 —— **稳定复现修正**（解决"起始位置错乱 / 三只手 / 贴墙瞬移"三个实战故障）：
1. 生成要求新增 6 条硬规则：【绝对空间】世界坐标系+机位登记表、【开局占位核对】、【肢体纪律】手部职责表、【禁用镜面反射】、【微动作禁位移】含光源自洽、【相机接力】。
2. 审核轮新增 ⑥肢体 / ⑦位置细节 两项检查。
3. 六字段规范新增第六十一节《稳定复现硬规则》，与第五十九节同优先级。
4. 详见 [CHANGELOG.md](./CHANGELOG.md)。
